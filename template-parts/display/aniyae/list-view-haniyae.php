...
<div class="w-full bg-gradient-to-t from-primary rounded shadow">
    <div class="block relative w-full group pb-72 sm:pb-80 md:pb-60 lg:pb-80 xl:pb-60 xxl:pb-[19rem]">
        ...
        <div class="flex items-center justify-between px-2 absolute bottom-0 inset-x-0">
            ...
            <span class="text-xs px-2 py-1 rounded-md font-semibold text-white" style="background-color: #ff0080" v-if="anime.latest">
                Ep <?php echo $desc_latest && $desc_latest['metadata']['number'] ? $desc_latest['metadata']['number'] : '?'; ?>
            </span>
        </div>

        <span class="absolute top-0 right-0 text-xs px-2 py-1 rounded-md font-semibold text-white" style="background-color: #ff0080"><?=!is_wp_error($status) && count($status) > 0 && $status[0] ? $status[0]->name : 'Finalizado';?></span>
