...
$per_page = get_theme_mod('__archive_count', 20);
$query = [
    'post_type' => 'anime',
            ...
            <form action="<?=Kiranime_Utility::page_url('pages/advanced-search.php')?>" method="GET">
                ...
                <div class="flex items-center flex-wrap gap-5">
                    ...
                    <div
                        ...
                        <label for="s_type" class="min-w-max flex-shrink-0">Tipo</label>
                    ...
                    <div
                        class="w-full lg:w-max lg:min-w-[200px] flex gap-2 items-center px-2 py-1 border-none ring-1 ring-sky-500 shadow-sm shadow-accent-3 drop-shadow-md focus:ring rounded-md text-sm">
                        <label for="s_season" class="min-w-max flex-shrink-0"><?php _e('Season', 'kiranime');?></label>
                        <select name="s_season" id="s_season"
                            ...
                            <option class="bg-tertiary py-2" value="winter"
                                <?php if ($season == 'winter') {echo 'selected';}?>><?php _e('Invierno', 'kiranime');?>
                            </option>
                            <option class="bg-tertiary py-2" value="spring"
                                <?php if ($season == 'spring') {echo 'selected';}?>><?php _e('Primavera', 'kiranime');?>
                            </option>
                            <option class="bg-tertiary py-2" value="summer"
                                <?php if ($season == 'summer') {echo 'selected';}?>><?php _e('Verano', 'kiranime');?>
                            </option>
                            <option class="bg-tertiary py-2" value="fall"
                                <?php if ($season == 'fall') {echo 'selected';}?>>
                                <?php _e('Otoño', 'kiranime');?></option>
                        </select>
                    </div>
                    <div
                        class="w-full lg:w-max lg:min-w-[200px] flex gap-2 items-center px-2 py-1 border-none ring-1 ring-sky-500 shadow-sm shadow-accent-3 drop-shadow-md focus:ring rounded-md text-sm">
                        <label for="s_year" class="min-w-max flex-shrink-0">Año</label>
                ...
                <button type="submit"
                    class="outline-none border-none shadow-lg drop-shadow-md rounded-md bg-accent-3 w-full block text-center text-sm font-medium col-span-full py-2 px-4 hover:shadow-xl hover:drop-shadow-lg hover:bg-sky-700 ease-in-out duration-200 transition-colors"><?php _e('Filstrar', 'kiranime');?></button>
            </form>
        </div>
        <div class="flex items-center justify-between">
            <h3 class="mb-4 text-2xl font-semibold leading-10 text-sky-400">
                <?php _e('Filter Results', 'kiranime');?>
                <?php if ($kw) {printf(esc_html__('For <i>%1$s</i>', 'kiranime'), $kw);}?>
            </h3>
            <span
